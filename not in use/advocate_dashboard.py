...
def api_put(endpoint, payload=None):
    r = requests.put(f"{API_URL}{endpoint}", json=payload)
    r.raise_for_status()
    return r.json()
...
def render_queue():
    ...
    for ticket in tickets:
        ...
        if status.lower() == "resolved":
            display_date = get_relative_time(ticket["ticket_id"])
        else:
            display_date = (
        ...
        st.markdown("<div class='ticket-row'>", unsafe_allow_html=True)

        c1, c2, c3 = st.columns([7, 2, 1])

        with c1:
            st.markdown(f"### {ticket['issue_title']}")
...
def render_ticket():
    ...
    ticket = api_get(f"/ticket/{ticket_id}")
    ...
    if st.button(
        ...
        st.session_state.resolution_text = (
            result.get("resolution_generated", "")
        )
